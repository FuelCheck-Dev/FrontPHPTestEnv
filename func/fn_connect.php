<?
	...
	if(@$_POST['cmd'] == 'login')
	{
		...
		else
		{
			...
			$q = "SELECT * FROM `gs_users` WHERE `username`='".$username."' AND `password`='".md5($password)."' AND `manager_id`='" .$companyId. "' LIMIT 1";		
			$r = mysqli_query($ms, $q);
	...
	function getCompanyId($company){
	    global $ms;
        $companyId = 0;
        writeLog('user_access', 'User login: company : "'.$company.'"');
        if($company == '') {
            $companyId = 0;
        }else{
            $manager_id = 0;
            $q = "SELECT * FROM `gs_users` WHERE `username`='" . $company . "' AND `manager_id`='" . $manager_id .  "' LIMIT 1";
            $r = mysqli_query($ms, $q);
            if ($row = mysqli_fetch_array($r)){
                $companyId =    $row['id'];
            }
