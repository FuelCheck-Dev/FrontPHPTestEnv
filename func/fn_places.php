<? 
	...
	{ 
		...
		$start = $limit*$page - $limit; // do not put $limit*($page - 1)
                ...
                if ($r && mysqli_num_rows($r) > 0)
                {
                        ...
                        {
                                $gid_clean = trim($gid, "'");
                                $group_counts[$gid_clean] = array('marker' => 0, 'route' => 0, 'zone' => 0);
                        }

                        $id_list = implode(',', $group_ids);

                        // get marker/route/zone counts in bulk
                        $q2 = "SELECT `group_id`, COUNT(*) AS `cnt` FROM `gs_user_markers` WHERE `group_id` IN (".$id_list.") GROUP BY `group_id`";
                        $r2 = mysqli_query($ms, $q2);
                        while ($r2 && ($row2 = mysqli_fetch_array($r2)))
                        {
                                $group_counts[$row2['group_id']]['marker'] = $row2['cnt'];
                        }

                        $q2 = "SELECT `group_id`, COUNT(*) AS `cnt` FROM `gs_user_routes` WHERE `group_id` IN (".$id_list.") GROUP BY `group_id`";
                        $r2 = mysqli_query($ms, $q2);
                        while ($r2 && ($row2 = mysqli_fetch_array($r2)))
                        {
                                $group_counts[$row2['group_id']]['route'] = $row2['cnt'];
                        }

                        $q2 = "SELECT `group_id`, COUNT(*) AS `cnt` FROM `gs_user_zones` WHERE `group_id` IN (".$id_list.") GROUP BY `group_id`";
                        $r2 = mysqli_query($ms, $q2);
                        while ($r2 && ($row2 = mysqli_fetch_array($r2)))
                        {
                                $group_counts[$row2['group_id']]['zone'] = $row2['cnt'];
                        }
                        ...
                        foreach ($groups as $row)
                        {
                                ...
                                $marker_number = $group_counts[$group_id]['marker'];
                                $route_number = $group_counts[$group_id]['route'];
                                $zone_number = $group_counts[$group_id]['zone'];
	...
	if(@$_POST['cmd'] == 'save_zone')
	{
		...
		if ($zone_id == 'false')
		{
			...
			$r = mysqli_query($ms, $q);
			$q = "SELECT `zone_id` FROM `gs_user_zones` WHERE `user_id`='".$user_id."' order by `zone_id` DESC LIMIT 1" ;
			$r = mysqli_query($ms, $q);
			if($row=mysqli_fetch_array($r)){
			    $zone_id =$row['zone_id'];
			   
			}
			updateSubUsers($user_id,$zone_id);
	...
	function updateSubUsers($user_id,$zone_id){
	    global $ms;
	    
	    $q = "SELECT * FROM `gs_users` WHERE  `manager_id`='".$user_id."'";
	    $r = mysqli_query($ms, $q);
	    ...
	    while($row=mysqli_fetch_array($r))
	    {
	        ...
	        $q1 = "UPDATE `gs_users` SET
							`privileges`='".$privileges."'
							WHERE `id`='".$subaccount_id."' AND `manager_id`='".$user_id."'";
	        $r1 = mysqli_query($ms, $q1);
	       
	    }

	    
	    if($_SESSION["privileges_zone"] == '""' ){
	        $_SESSION["privileges_zone"] = '"'.$zone_id.'"';
	    }else{
	    $_SESSION["privileges_zone"] = $_SESSION["privileges_zone"].',"'.$zone_id.'"';
	    }
