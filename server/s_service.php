<?
	...
	if (@$_GET["op"] == "sms_gateway_app")
  	{
  	    writeLog('server', 'op=sms_gateway_app');
	...
	if (@$_GET["op"] == "chat_new_messages")
  	{
  	    writeLog('server', 'op=chat_new_messages');
		$imei = $_GET["imei"];
	...
	if (@$_GET["op"] == "push_new_events")
  	{
  	    writeLog('server', 'op=push_new_events');
	...
	if ((@$_GET["op"] == "object_exists_system") || (@$_GET["op"] == "check_object_exists_system"))
  	{
  	    writeLog('server', 'op=object_exists_system || check_object_exists_system');
	...
	if (@$_GET["op"] == "cmd_exec_imei_get")
  	{
  	    writeLog('server', 'op=cmd_exec_imei_get');
		$format = strtolower(@$_GET["format"]);
	...
	{
	    writeLog('server', 'dataOp=clear_object_history');
		clearObjectHistory($data['imei']);
	}
	
	if (@$data["op"] == "get_cmd_exec")
  	{
  	    writeLog('server', 'dataOp=get_cmd_exec');
		$q = "SELECT gs_objects.*, gs_object_cmd_exec.*
			FROM gs_objects
			INNER JOIN gs_object_cmd_exec ON gs_objects.imei = gs_object_cmd_exec.imei
	...
	if (@$data["op"] == "set_cmd_exec")
  	{
  	    writeLog('server', 'dataOp=set_cmd_exec');
	...
	{
	    writeLog('server', 'dataOp=service_12h');
		serviceServerCleanup();
	}
	
	if (@$data["op"] == "service_1h")
	{
	    writeLog('server', 'dataOp=service_1h');
	...
	{
	    writeLog('server', 'dataOp=service_30min');
	...
	{
	    writeLog('server', 'dataOp=service_5min');
		serviceMileageDaily();
		serviceCMDSchedule();
		serviceEventService();
		serviceDbBackup();
	...
	{
	    writeLog('server', 'dataOp=service_1min');
		...
		for($i = 0; $i < $reports_count; $i++)
		{
			$postdata = http_build_query(
							array(
							    ...
							    'optimal_consumption' => $reports[$i]['optimal_consumption'],
							    'drivers' => $reports[$i]['drivers'],
								'dtf' => $reports[$i]['dtf'],
								'dtt' => $reports[$i]['dtt']
							));
