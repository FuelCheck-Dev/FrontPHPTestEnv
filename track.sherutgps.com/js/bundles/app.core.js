/*! App core bundle: gs.config.js, gs.common.js, gs.connect.js (shared bootstrap utilities). */
// ############################################################
...
function transformToEventData(data){
...
{
	events.push({
		type: currentData[i][0],
		speed: currentData[i][1],
		driver: currentData[i][2],
		lat: currentData[i][3],
		lng: currentData[i][4],
		time: currentData[i][5],
		address: currentData[i][6]
		});
		
	}
mapdata.set(key,events);	
	}
	return mapdata;
}

function transformToEventData2(data){
...
// #################################################
var gsPerfAjaxTracker = {
    requests: {}
};

function gsPerfInitAjaxMetrics()
{
    if (typeof jQuery == 'undefined' || gsPerfAjaxTracker.initialized === true)
    {
        return;
    }

    gsPerfAjaxTracker.initialized = true;

    jQuery(document).ajaxSend(function(event, jqXHR, settings) {
        var started = (window.performance && performance.now) ? performance.now() : new Date().getTime();
        var reqId = 'rq_' + started + '_' + Math.floor(Math.random() * 1000000);
        jqXHR.__gs_req_id = reqId;
        gsPerfAjaxTracker.requests[reqId] = {
            started: started,
            url: settings.url,
    ...
    jQuery(document).ajaxComplete(function(event, jqXHR, settings) {
        var reqId = jqXHR.__gs_req_id;
        if (!reqId || !gsPerfAjaxTracker.requests[reqId])
        {
            return;
        }

        var info = gsPerfAjaxTracker.requests[reqId];
        delete gsPerfAjaxTracker.requests[reqId];

        var ended = (window.performance && performance.now) ? performance.now() : new Date().getTime();
        var requestMs = Math.round(ended - info.started);
        var payloadSize = 0;
        if (typeof info.payload == 'string')
        {
            payloadSize = info.payload.length;
        }
        else if (info.payload)
        {
            payloadSize = JSON.stringify(info.payload).length;
        }

        var responseText = jqXHR.responseText || '';
        var responseSize = responseText.length;
        var cmd = '';
        if (typeof info.payload == 'string' && info.payload.indexOf('cmd=') != -1)
        {
            cmd = info.payload;
        }
        ...
        var metric = {
            endpoint: info.url,
            cmd: cmd,
            t_request_ms: requestMs,
            payload_size: payloadSize,
            response_size: responseSize,
            status: jqXHR.status,
            request_id: jqXHR.getResponseHeader('X-Request-Id') || ''
        };

        if (window.console && console.log)
        {
            console.log('[GS_PERF]', metric);
        }

        if (window.gsPerfConfig && gsPerfConfig.beacon_url)
        {
            try
            {
                if (navigator.sendBeacon)
                {
                    navigator.sendBeacon(gsPerfConfig.beacon_url, JSON.stringify(metric));
                }
            }
            catch (e) {}
        }
...
{
    jQuery(function() {
        gsPerfInitAjaxMetrics();
    });
}
// #################################################
// END PERFORMANCE TELEMETRY
// #################################################
