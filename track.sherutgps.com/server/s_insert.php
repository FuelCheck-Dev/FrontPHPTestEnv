<?
	...
	//$_POST['event'] - possible events: sos, bracon, bracoff, dismount, door, mandown, shock, tow, pwrcut, gpscut, jamming, lowdc, lowbat, haccel, hbrake, hcorn
					...
					if ((strtotime($loc['dt_tracker']) > 0) && (strtotime($loc_prev['dt_tracker']) > 0))
					{
						...
						if (isset($params_prev[$acc]) && isset($params[$acc]))
						{
							if (($params_prev[$acc] == '1') && ($params[$acc] == '1'))
							{
								...
								// calculate engine hours only if message time difference is not longer than 120 sec in order to avoid issues with some devices which change parameters without location data
								if ($engine_hours <= 120)
								{
