<?
	...
	function check_events($loc, $loc_prev, $loc_events, $params_events, $service_events)
	{
			...
			{			
				...
				{					
					...
					if (!isset($loc['event']))
					{
						...
						if($loc['protocol'] == 'ruptela'){
					    set_ruptela_events($loc,$ed);    
					...
					if (($ed['type'] == 'econbrake') && ($loc['event'] == 'econbrake'))
					{
					...
					if (($ed['type'] == 'ecohaccel') && ($loc['event'] == 'ecohaccel'))
					{
					...
					if (($ed['type'] == 'ecoexthbra') && ($loc['event'] == 'ecoexthbra'))
					{
		...
		function set_ruptela_events(&$loc,$ed){
	    
	    if ($ed['type'] == 'ecohaccel' && isset($loc['params']['io136']) )
	    {
	        $ecohaccel = $loc['params']['io136'];
	    ...
	    if ($ed['type'] == 'econbrake' && isset($loc['params']['io134']) )
	    {
	        $econbrake = $loc['params']['io134'];
	    ...
	    if ($ed['type'] == 'ecoexthbra' && isset($loc['params']['io135']) )
	    {
	        $ecoexthbra = $loc['params']['io135'];
	        
	        if($ecoexthbra > 0){
	            $loc['event'] = 'ecoexthbra';
	        }
