<?
	...
	function gsPerfStart($endpoint, $cmd = '')
	{
		$rid = '';
	...
	function gsPerfTrackDb(&$perf, $queryStart, $rows = 0)
	{
		$perf['db_time'] += microtime(true) - $queryStart;
		$perf['db_rows'] += (int)$rows;
	}

	function gsPerfFinish($perf, $payload = null, $extra = array())
	{
		$payloadSize = 0;
		...
		$log = array(
			...
			'cmd' => $perf['cmd'],
			'total_time_ms' => round((microtime(true) - $perf['t_start']) * 1000, 2),
			'db_time_ms' => round($perf['db_time'] * 1000, 2),
			'rows' => $perf['db_rows'],
		...
		$logPath = dirname(__FILE__).'/../logs/perf.log';
...
", FILE_APPEND | LOCK_EX);
	...
	function checkUsernameExists2($username,$manager_id)
{
    global $ms;

    $username = strtolower($username);

    $q = "SELECT * FROM `gs_users` WHERE `username`='".$username."' AND `manager_id`='".$manager_id."' LIMIT 1";
    $r = mysqli_query($ms, $q);
    $num = mysqli_num_rows($r);
	...
	function checkEmailExists2($email, $manager_id)
{
    global $ms;

    $email = strtolower($email);
    writeLog('user_access', 'yariv: email: '.$email. ' manager ' .$manager_id. ' ');
    $q = "SELECT * FROM `gs_users` WHERE `email`='".$email."' AND `manager_id`='".$manager_id."' LIMIT 1";
    $r = mysqli_query($ms, $q);
    $num = mysqli_num_rows($r);
	...
	function addUser($send, $active, $account_expire, $account_expire_dt, $privileges, $manager_id, $username, $email, $password, $obj_add, $obj_limit, $obj_limit_num, $obj_days, $obj_days_num, $obj_edit, $obj_delete, $obj_history_clear)
	{
		...
		if (!checkEmailExists2($email, $manager_id))
		{
			if ($username == '')
			{
				$username = $email;
			...
			if (!checkUsernameExists2($username,$manager_id))
			{
				if ($password == '')
				{
	...
	function  getNumFromByte($value,$start,$end){
	    if($value < 0 || $value >255 || $start < 0 || $start > 8 ||   $end < 0 || $end > 8 || $start > $end){
	        return 0;
	    }
	    $bin = sprintf( "%08d", decbin($value ));
	    return bindec(substr($bin,$start,$end));
	...
	function getTimeDetailsObj($sec, $show_days)
	{
