<? 
	...
	{
		$perf = gsPerfStart('fn_cmd.php', 'load_cmd_gprs_exec_list');
		$page = max(1, (int)$_GET['page']); // get the requested page
		...
		$scope = ($_SESSION["privileges"] == 'subuser') ? $_SESSION["privileges_imei"] : getUserObjectIMEIs($user_id);
		$q_count = "SELECT COUNT(*) as cnt FROM `gs_object_cmd_exec` WHERE `imei` IN (".$scope.") AND `gateway`='gprs'";
		...
		$offset = ($page - 1) * $limit;
		$q = "SELECT * FROM `gs_object_cmd_exec` WHERE `imei` IN (".$scope.") AND `gateway`='gprs' ORDER BY $sidx $sord LIMIT $offset, $limit";
		...
		gsPerfFinish($perf, $json, array('records' => $count, 'page' => $page, 'limit' => $limit));
	...
	{
		$perf = gsPerfStart('fn_cmd.php', 'load_cmd_sms_exec_list');
		$page = max(1, (int)$_GET['page']); // get the requested page
		$limit = (int)$_GET['rows']; // get how many rows we want to have into the grid
		if ($limit < 1) { $limit = 50; }
		if ($limit > 500) { $limit = 500; }
		...
		$scope = ($_SESSION["privileges"] == 'subuser') ? $_SESSION["privileges_imei"] : getUserObjectIMEIs($user_id);
		$q_count = "SELECT COUNT(*) as cnt FROM `gs_object_cmd_exec` WHERE `imei` IN (".$scope.") AND `gateway`='sms'";
		$q_count_start = microtime(true);
		$r_count = mysqli_query($ms, $q_count);
		gsPerfTrackDb($perf, $q_count_start, $r_count ? 1 : 0);
		if (!$r_count){die;}
		...
		$count = (int)$row_count['cnt'];
		$offset = ($page - 1) * $limit;
		$q = "SELECT * FROM `gs_object_cmd_exec` WHERE `imei` IN (".$scope.") AND `gateway`='sms' ORDER BY $sidx $sord LIMIT $offset, $limit";
		$q_start = microtime(true);
		$r = mysqli_query($ms, $q);
		gsPerfTrackDb($perf, $q_start, $r ? mysqli_num_rows($r) : 0);
