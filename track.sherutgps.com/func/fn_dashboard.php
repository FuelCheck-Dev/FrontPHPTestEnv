<? 
	...
	{
		$perf = gsPerfStart('fn_dashboard.php', 'load_events_data');
		$result = array();
		...
		if ($_SESSION["privileges"] == 'subuser')
		{
			$q = "SELECT e.* FROM `gs_user_last_events_data` e
			INNER JOIN `gs_objects` o ON o.imei = e.imei AND o.active='true'
			WHERE e.`user_id`='".$user_id."' AND e.`imei` IN (".$_SESSION["privileges_imei"].")";
		}
		else
		{
			$q = "SELECT e.* FROM `gs_user_last_events_data` e
		...
		$q .= " AND e.dt_tracker BETWEEN '".convUserUTCTimezone($dtf)."' AND '".convUserUTCTimezone($dtt)."'";
		...
		{				
			...
			else
			{
				...
				else if ($type == 'econbrake')
				{
				    $name = $la['ECO_NORMAL_BRAKING'];
				}
				else if ($type == 'hbrake')
				{
					$name = $la['HARSH_BRAKING'];
				}
				else if ($type == 'ecohaccel')
				{
				    $name = $la['ECO_HARSH_ACCELERATION'];
				}
				else if ($type == 'ecoexthbra')
				{
				    $name = $la['ECO_EXTREME_AND_HARSH_BRAKE'];
				}
				else if ($type == 'hcorn')
				{
		...
		gsPerfFinish($perf, $json, array('range' => $dtf.'|'.$dtt, 'types' => count($result)));
