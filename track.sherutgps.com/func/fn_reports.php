<? 
	...
	if(@$_POST['cmd'] == 'load_report_data')
	{
		...
		while($row=mysqli_fetch_array($r))
		{
			...
			$result[$report_id] = array(	'name' => $row['name'],
							...
							'filter_reports'=>$row['filter_reports'],
							'format' => $row['format'],
			                ...
			                'optimal_consumption' => $row['optimal_consumption'],
			                'min_duration_zone' => $row['min_duration_zone'],
							'imei' => $row['imei'],
			                'drivers' => $row['drivers'],
	...
	if(@$_POST['cmd'] == 'save_report')
	{
		...
		$filter_reports=$_POST['filter_reports'];
		$format = $_POST["format"];
...
if(@$_POST['cmd'] == 'delete_selected_reports_background_generated')
{
    $items = $_POST["items"];
    ...
    for ($i = 0; $i < count($items); ++$i) {
        ...
        deleteReportBackground($report_id, $user_id);
    }
    echo 'OK';
    die;
}

if(@$_POST['cmd'] == 'delete_report_background_generated')
{
    $report_id = $_POST["report_id"];
...
if(@$_GET['cmd'] == 'load_reports_generated_background_list')
{
    $page = $_GET['page']; // get the requested page
    $limit = $_GET['rows']; // get how many rows we want to have into the grid
    $sidx = $_GET['sidx']; // get index row - i.e. user click to sort
    ...
    // get records number
    $q = "SELECT * FROM `gs_user_reports_background` WHERE `user_id`='".$user_id."'";
    $r = mysqli_query($ms, $q);
    $count = mysqli_num_rows($r);
    ...
    $q = "SELECT * FROM `gs_user_reports_background` WHERE `user_id`='".$user_id."' ORDER BY $sidx $sord LIMIT $start, $limit";
    $r = mysqli_query($ms, $q);
    ...
    if ($r)
    {
        ...
        while($row = mysqli_fetch_array($r))
        {
            ...
            $modify = '';
            if($row['status'] == 2) {
                $modify .= '<a href="#" onclick="reportsBackgroundDownload(\'' . $report_id . '\');" title="' . $la['DOWNLOAD'] . '"><img src="theme/images/file.svg" /></a>';
                $modify .= '<a href="#" onclick="reportsGeneratedBackgroundDelete(\'' . $report_id . '\');" title="' . $la['DELETE'] . '"><img src="theme/images/remove3.svg" /></a>';
            }
            if($row['status'] == -1){
                $modify .= '<a href="#" onclick="reportsGeneratedBackgroundDelete(\'' . $report_id . '\');" title="' . $la['DELETE'] . '"><img src="theme/images/remove3.svg" /></a>';
            ...
            // set row
            $response->rows[$i]['id']=$report_id;
            $response->rows[$i]['cell']=array($dt_report,$name,$type,$format,$status,$modify);
            $i++;
        }
    }
...
if(@$_POST['cmd'] == 'download_background_report'){

    $report_id = $_POST["report_id"];

    $q = "SELECT * FROM `gs_user_reports_background` WHERE `report_id`='".$report_id."'";
    $r = mysqli_query($ms, $q);
    $row = mysqli_fetch_array($r);

    $report_file = $gsValues['PATH_ROOT'].'data/user/reports/'.$row['file_path'];
    $report = '';
    if(is_file($report_file))
    {
        $report = file_get_contents($report_file);
    ...
    $result = array('format' => $row['format'], 'filename' => $row['name'], 'content' => $report);
