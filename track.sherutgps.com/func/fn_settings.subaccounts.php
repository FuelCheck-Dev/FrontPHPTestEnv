<?
	...
	if(@$_POST['cmd'] == 'save_subaccount')
	{
		...
		else
		{
			...
			$q = "SELECT * FROM `gs_users` WHERE `username`='".$username."' AND `manager_id`='".$manager_id."' AND `id`<>'".$subaccount_id."' LIMIT 1";
			$r = mysqli_query($ms, $q);
			...
			$q = "SELECT * FROM `gs_users` WHERE `email`='".$email."' AND `manager_id`='".$manager_id."' AND `id`<>'".$subaccount_id."' LIMIT 1";
			$r = mysqli_query($ms, $q);
			$num = mysqli_num_rows($r);
