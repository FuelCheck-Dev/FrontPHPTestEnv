<?
	...
	function getRouteFuelConsumption2($route,$imei){
	    ...
	    return $result['fuel_consumption'];
	}
	...
	function getRouteFuelTankById($route, $accuracy, $fuel_sensor){
	    $result = array();
	    $result['fill_theft'] = array();
	    
	    if ($fuel_sensor == false)
	    {
	        return $result;
	    }
	    ...
	    $total_filled = 0;
	        ...
	        for ($i=0; $i<count($route)-1; ++$i)
	        {
	            ...
	            $fuel_current = getSensorValue($params1, $fuel_sensor); // fuel level
	            $fuel_next = getSensorValue($params2, $fuel_sensor); // fuel level in next point
	            ...
	            else
	            {
	                if ($filling == 1)
	                {
	                    ...
	                    $result['fill_theft'][] = array(	$filling_dt,
	                        $filling_lat,
	                        $filling_lng,
	                        $filling_before,
	                        $filling_after,
	        ...
	        for ($i=0; $i<count($route)-1; ++$i)
	        {
	            ...
	            else
	            {
	                if ($theft == 1)
	                {
	                    ...
	                    $result['fill_theft'][] = array(	$theft_dt,
	                        $theft_lat,
	                        $theft_lng,
	                        $theft_before,
	                        $theft_after,
	    ...
	    usort($result['fill_theft'], function($a, $b) {
	...
	function getRouteFuelTank($route, $accuracy, $fuel_sensors){
	    $result = array();
	    $result['fill_theft'] = array();
	  ...
	  $unit=  $fuel_sensors[0]['units'];
	    $filling = 0;
	        ...
	        for ($i=0; $i<count($route)-1; ++$i)
	        {
	            ...
	            $fuel_current_total += $fuel_current['value'];
	            $fuel_next_total +=$fuel_next['value'];
	            }
	            $diff = $fuel_next_total - $fuel_current_total; // fuel filling
	            ...
	            if (($diff >= $diff_ff) && (($speed1 < $min_fuel_speed) || ($speed2 < $min_fuel_speed)) && ($i < count($route)-2))
	            {
	                if ($filling == 0)
	                {
	                    ...
	                    $filling_before =  $fuel_current_total;
	            ...
	            else
	            {
	                if ($filling == 1)
	                {
	                    $filling_after =  $fuel_current_total;
	        ...
	        for ($j=0; $j<count($fuel_sensors); ++$j)
	        {
	            for ($i=0; $i<count($route)-1; ++$i)
	            {
	                ...
	                $diff =  $fuel_current_total - $fuel_next_total; // fuel theft
	                ...
	                if (($diff >= $diff_ft) && (($speed1 < $min_fuel_speed) || ($speed2 < $min_fuel_speed)) && ($i < count($route)-2))
	                {
	                    if ($theft == 0)
	                    {
	                        ...
	                        $theft_before = $fuel_current_total;
	                ...
	                else
	                {
	                    if ($theft == 1)
	                    {
	                        $theft_after = $fuel_current_total;
